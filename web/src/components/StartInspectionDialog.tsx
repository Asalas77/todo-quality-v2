...
import type { StartInspectionInput } from '../api/inspecciones';
...
export function StartInspectionDialog({
  ...
  } = useForm<StartInspectionInput>({ resolver: zodResolver(schema) });

  useEffect(() => {
    if (!open) return;
    setServerError(null);
    reset({ templateId: '', centroId: '', fecha: new Date().toISOString().slice(0, 10) });
  ...
  const submit = handleSubmit(async (values) => {
    setServerError(null);
    try {
      const inspection = await inspeccionesApi.start(values);
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <FormControl fullWidth error={!!errors.templateId}>
              ...
              <Select labelId="template-label" label="Plantilla" defaultValue="" {...register('templateId')}>
                {templates.map((t) => (
